...
import juitar.sweet_charm_o_mine.network.SwitchPocketPacket;
...
public class PocketScreen extends AbstractContainerScreen<PocketContainer> {
    ...
    protected void init() {
        super.init();
        if (!this.menu.hasSwitcher()) {
            return;
        }

        int buttonY = this.topPos + 5;
        this.addRenderableWidget(Button.builder(Component.literal("<"),
                button -> SweetCharmNetwork.CHANNEL.sendToServer(new SwitchPocketPacket(-1)))
                .bounds(this.leftPos + this.imageWidth - 40, buttonY, 12, 12)
                .build());
        this.addRenderableWidget(Button.builder(Component.literal(">"),
                button -> SweetCharmNetwork.CHANNEL.sendToServer(new SwitchPocketPacket(1)))
                .bounds(this.leftPos + this.imageWidth - 20, buttonY, 12, 12)
                .build());
    }
